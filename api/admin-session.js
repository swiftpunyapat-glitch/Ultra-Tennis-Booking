...
export default async function handler(req, res) {
  ...
  if (action === 'login') {
    ...
    // ── Owner/admin via ADMIN_USERS_JSON only ───────────────────────
    // Coach V2: coach PIN login is RETIRED — coaches sign in with LINE on
    // coach.html (token auth), never through this route.
    if (!getAdminUser(name) || !validateAdminPin(name, pin)) {

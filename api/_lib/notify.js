...
วันที่: ${p.date || "—"}
เวลา: ${p.startTime || "—"}–${p.endTime || "—"}
ประเภท: ${p.bookingType || "—"}
ชำระ: ${p.paymentStatus || "—"}
โดย: ${p.actionBy || "—"}${p.note ? `
โน้ต: ${p.note}` : ""}`,
  }],
...
เบอร์: ${p.customerPhone || "—"}
รหัส: ${p.bookingCode || "—"}
เดิม: ${p.previousDate || "—"} ${p.previousStartTime || "—"}–${p.previousEndTime || "—"}
ใหม่: ${p.date || "—"} ${p.startTime || "—"}–${p.endTime || "—"}
ประเภท: ${p.bookingType || "—"}
ชำระ: ${p.paymentStatus || "—"}
โดย: ${p.actionBy || "—"}`,
  }],
...
เบอร์: ${p.customerPhone || "—"}
รหัส: ${p.bookingCode || "—"}
คืนเงิน: ฿${p.refundAmount || "—"}
เหตุผล: ${p.refundReason || "—"}
เหตุการณ์: ${p.incidentType || "—"}
เล่น: ${p.date || "—"} ${p.startTime || "—"}–${p.endTime || "—"}
โดย: ${p.actionBy || "—"}`,
  }],

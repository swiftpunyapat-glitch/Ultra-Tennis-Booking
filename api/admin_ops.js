...
function slotGuardSent(res, session) {
  ...
  // HQ roles are global by definition. Do not require an explicit branch entry
  // in a legacy/admin session for the default Ultra branch.
  if (!isHQ(session) && !hasBranchAccess(session, DEFAULT_BRANCH_ID)) {

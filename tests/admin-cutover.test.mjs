...
beforeAll(async()=>{
  ...
  cookies=Object.fromEntries(['Art','Staff','Other','View'].map(n=>[n,createSessionCookie(n).split(';')[0]]));
});
...
describe('admin_read authentication, scope, limits, and projections',()=>{
  ...
  test('authorized slip-review role sees allowlisted PII and slip URL only in its branch',async()=>{
    const out=await call(opsHandler,{action:'admin_read',resource:'bookings',limit:20},'Staff');
    expect(out.statusCode).toBe(200);
    expect(out.body.items.every(x=>x.branchId==='ladprao1')).toBe(true);
    expect(out.body.items[0]).toHaveProperty('customerName');
    expect(out.body.items[0]).toHaveProperty('slipUrl');
    expect(out.body.items[0]).not.toHaveProperty('secretUnprojectedField');
  });
});
...
describe('admin operational mutation matrix',()=>{
  ...
  test('package adjustment and deactivation are branch-scoped and audited',async()=>{
    ...
    expect((await call(userActionHandler,adjust,'Other')).statusCode).toBe(403);
    expect((await call(userActionHandler,adjust,'Staff')).statusCode).toBe(200);
    expect((await db.collection('customer_packages').doc('pkg1').get()).data().remainingMinutes).toBe(120);
    expect((await call(userActionHandler,{action:'deactivate_pass',passId:'pkg1'},'Other')).statusCode).toBe(403);
    expect((await call(userActionHandler,{action:'deactivate_pass',passId:'pkg1'},'Staff')).statusCode).toBe(200);
